...
//初始化页面
(function library() {
  fetch(`http://localhost:3000/modul/initializeLibrary?user_id=${userId}`, {
    method: "get",
    headers: {
      "Content-Type": "application/json",
    },
    ...
    .then((response) => response.json())
    .then((responseData) => {
      console.log("初始化库成功", responseData);
      // 正确的数据访问方式
      console.log("完整响应数据:", responseData); // 添加调试日志
      const actualData = responseData.data || []; // 确保有默认值
      const library = document.querySelector(".main .left-box .list ul");
      let str = "";
      actualData.forEach((item) => {
        // 确保item存在且包含必要属性
        if (!item) return;
        const modelHead = item.model_head || "未命名";
        const modelData = item.model_data.content || "暂无描述";
        str += `
          <li>
              <div class="item">
                <div class="left">
              ...
              <div class="dropdown hide">
                <div class="item" draggable="true" data-type="node">
                  <div class="top">
                    <div class="left">
                      <svg
                        t="1743336869398"
      ...
      library.insertAdjacentHTML("afterbegin", str);
    })
    .catch((error) => {
      console.error("初始化库失败", error);
    });
})();
